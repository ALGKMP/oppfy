...
const CameraPage = () => {
  ...
  const captureButtonRef = useRef<any>(null);
  ...
  const player = useVideoPlayer(
    require("@assets/videos/indiana jones.mp4"),
    (player) => {
  ...
  // Listen for video end and route to post-to
  useEffect(() => {
    const subscription = player.addListener("playToEnd", () => {
      // Video finished playing, prepare the video asset and route
      const videoAsset = require("@assets/videos/indiana jones.mp4");
      const processedUri =
  ...
  // Cleanup: pause video when component unmounts or screen loses focus
  useEffect(() => {
    return () => {
      player.pause();
      player.muted = true;
  ...
  // Additional cleanup when screen loses focus
  useEffect(() => {
    if (!isActive) {
      player.pause();
    } else if (isActive) {
